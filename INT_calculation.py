...
#   to compute for different distances, set the chosen distance in lines: 73, 80, 99, 111, 114, 115 and 117
...
for dist in [LD, FDL, FDK]:
    ...
    for child_id in child_ids:
        ...
        for r in range(len(child.index)):
            ...
            row_count += 1
    if dist_count == 1:
        INT_values_LD_df.to_csv('./csv_files/' + 'INT_values_LD_df.csv')
    elif dist_count == 2:
        INT_values_FDL_df.to_csv('./csv_files/' + 'INT_values_FDL_df.csv')
    else:
        INT_values_FDK_df.to_csv('./csv_files/' + 'INT_values_FDK_df.csv')
